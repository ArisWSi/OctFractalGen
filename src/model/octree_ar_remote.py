...
class ChildOctreeInfo:
    ...
    def _calc_mask(self, group, cond="neq"):
        """根据分组张量计算 attention mask（float，与 OctGPT 兼容）。

        返回 (P, K, K) float mask，-1e3 = 屏蔽，0 = 允许。
        """
        K = self.patch_size
        group = group.view(-1, K)  # (P, K)
        same_batch = group.unsqueeze(2) == group.unsqueeze(1)  # (P, K, K)
        valid = (group != -1)
        valid_mask = valid.unsqueeze(2) & valid.unsqueeze(1)
        allowed = same_batch & valid_mask

        if cond == "le":
            pass  # 单深度，无 teacher-forcing

        # float mask: -1e3 = 屏蔽, 0 = 允许
    ...
    def _build_masks(self):
        ...
        self.dilate_mask = self._calc_mask(batch_dilate)

        if self.use_swin:
            b_idx_swin = self._pad_swin(self.batch_idx[:self.nnum_t])
            self.swin_patch_mask = self._calc_mask(b_idx_swin)
            b_dil_swin = self._pad_swin(batch_dilate[:self.nnum_t.item()])
            self.swin_dilate_mask = self._calc_mask(b_dil_swin)
